#
# Body temperature (Tb) and evaporative water loss (EWL) calculation (for Boyaca) 
#


# working directory
setwd("C:/Users/luism/OneDrive/Escritorio/curso/Dia 4 - Junio 18/04_pract/2_5m") # set your own directory
...
ground_0cm_ras <- rast("cur_microclim/Soil(subst_temp)/D0cm_soil_all.nc")[[19]] # layer 19 = 19:00 h, January
RH_ras <- rast("cur_microclim/RH(relat_hum)/RH1cm_all.nc")[[19]]                # layer 19 = 19:00 h, January
solr_radt_ras <- rast("cur_microclim/SOLR/SOLR_all.nc")[[19]]                   # layer 19 = 19:00 h, January
tair_ras <- rast("cur_microclim/TA(air_Temp)/TA1cm_all.nc")[[19]]               # layer 19 = 19:00 h, January
wind_ras <- rast("cur_microclim/V1cm/V1cm_all.nc")[[19]]                        # layer 19 = 19:00 h, January
...
mechanistic_Tb[micro_env2$cell]  <- micro_env2$model_Tb # now fill the empty raster with the calculated Tb
...
ggplot() +
  ...
  labs(x = "Longitude", 
       y = "Latitude", 
       title = "Estimated Body Temperature at 19:00 h, January") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))

# export
ggsave(filename = "mechanistic_Tb.jpg", wi = 16, he = 17, un = "cm", dpi = 200)
...
ggplot() +
  ...
  labs(x = "Longitude", 
       y = "Latitude", 
       title = "Estimated Evaporative Water Loss at 19:00 h, January") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))

# export
ggsave(filename = "mechanistic_ewl.jpg", wi = 16, he = 17, un = "cm", dpi = 200)
