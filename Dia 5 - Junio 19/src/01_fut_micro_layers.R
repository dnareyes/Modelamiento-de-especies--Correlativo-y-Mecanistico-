...
base_dir <- "C:/Users/luism/OneDrive/Escritorio/curso/Dia 5 - Junio 19/05_pract"

## working directory
path_wd <- file.path(base_dir, "02_my_fut_microclim")

# boyacá raster model
path_range_sp <- "data/boyaca_ras_2_5m.tif"

# my created global climate (for Boyaca actually) for the future
path_my_global <- file.path(base_dir, "01_my_global_climate_fut", "data", "my_global")

# path to copy my current global climate
path_path <- file.path(path_wd, "path")
...
# now replace the just downloaded global climate by our own (custom-made) current global climate
file.copy(file.path(path_my_global, "global_climate.nc"), path_path, overwrite = TRUE, copy.mode = TRUE)



## Get coordinates to calculate current microclimates

# get your global climate just to get a raster reference of our target region
global_sp <- raster::stack(list.files(path_path, pattern = "global_climate*.*nc", full.names = TRUE))[[1]]
plot(global_sp$X1)

# mask to our specific target region (Departamnto de Boyaca)
ras_mod <- raster::raster(path_range_sp)
plot(ras_mod$wc2.1_2.5m_tmax_01)
...
# get the microenvironmental conditions
...
# create directory
dir.create("fut_microclim", showWarnings = FALSE)
dir.create("fut_microclim/Soil(subst_temp)", showWarnings = FALSE)
dir.create("fut_microclim/TA(air_Temp)", showWarnings = FALSE)
dir.create("fut_microclim/RH(relat_hum)", showWarnings = FALSE)
dir.create("fut_microclim/V1cm", showWarnings = FALSE)
dir.create("fut_microclim/SOLR", showWarnings = FALSE)
...
terra::writeCDF(terra::rast(sp_D0cm_25_all), 
                filename = "fut_microclim/Soil(subst_temp)/D0cm_soil_all.nc", 
...
terra::writeCDF(terra::rast(sp_TALOC_25_all), 
                filename = "fut_microclim/TA(air_Temp)/TA1cm_all.nc", 
...
terra::writeCDF(terra::rast(sp_RHLOC_25_all), 
                filename = "fut_microclim/RH(relat_hum)/RH1cm_all.nc", 
...
terra::writeCDF(terra::rast(sp_VLOC_25_all), 
                filename = "fut_microclim/V1cm/V1cm_all.nc", 
...
terra::writeCDF(terra::rast(sp_SOLR_25_all), 
                filename = "fut_microclim/SOLR/SOLR_all.nc", 
